...
<style>
  .filter-tab {
    padding:.4rem 1.1rem; border-radius:20px; font-size:.82rem; font-weight:600;
  ...
  .filter-badge { background:rgba(255,255,255,0.25); border-radius:10px; font-size:.68rem; padding:1px 6px; margin-left:3px; }
  .filter-tab:not(.active) .filter-badge { background:#f3f4f6; color:#6b7280; }

  .ev-row {
    display:flex; align-items:center; gap:1rem;
    padding:1rem 1.25rem; border-bottom:1px solid #f3f4f6; transition:background .12s;
  }
  .ev-row:last-child { border-bottom:none; }
  .ev-row:hover { background:#f8fffd; }
  .ev-thumb {
    width:72px; height:50px; border-radius:8px; flex-shrink:0; overflow:hidden;
  ...
  .qa-btn { display:inline-flex; align-items:center; gap:.5rem; padding:.6rem 1.1rem;
    border-radius:10px; font-size:.84rem; font-weight:600; text-decoration:none; border:1.5px solid transparent; transition:all .16s; }
  ...
  [data-bs-theme="dark"] .filter-tab:not(.active):hover { border-color:#0F6E56; color:#5DCAA5; }
...
<div class="d-flex justify-content-between align-items-center mb-4">
  <div>
    ...
    <p class="text-muted mb-0" style="font-size:.81rem;">
      <?= $statusCounts['all'] ?> event<?= $statusCounts['all'] != 1 ? 's' : '' ?> total
    </p>
  </div>
